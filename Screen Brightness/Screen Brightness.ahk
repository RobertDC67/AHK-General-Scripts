...
;;∙------∙Global variables to store monitor handle and brightness range.
...
InitMonitor() {
    ...
    ;;∙------∙Prepare structure to receive physical monitor info.
    structSize := A_PtrSize + 256
    VarSetCapacity(physicalMonitors, structSize, 0)

    ;;∙------∙Retrieve physical monitors.
    ...
    ;;∙------∙Extract the handle of the first physical monitor.
    ...
    ;;∙------∙Store min/max brightness globally.
...
AdjustBrightness(step) {
    ...
    ;;∙------∙Get CURRENT brightness (refresh it every time).
    current := 0
