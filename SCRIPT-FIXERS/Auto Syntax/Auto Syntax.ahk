...
» Auto Syntax Tidy v13
» Change indentation and case of commands according to syntax by toralf & hajos.
» See all notes at bottom of script...
» SOURCE :  https://www.autohotkey.com/boards/viewtopic.php?t=85707
...
#SingleInstance, Force 		 ; Originally set to 'Ignore'
SetBatchLines, -1
...
; "Fix the working directory as the script directory, and the Syntax folder will naturally be fixed in it."
AHKPath:=A_ScriptDir
IfNotExist %AHKPath%
...
ReadDataFromIni:
	IniRead, Extension, %IniFile%, Settings, Extension, _Syntaxed %Version%.ahk
...
» Known limitations:
    ...
    ⮞ comments might not have the right indentation in certain strongly encapsulated block-structures, due to not knowing what the next line will bring.
    ...
    ⮞ After doing indentation in an editor the cursor jumps to the first position in the last line.
    ⮞ Indentation might fail, if a "{" is the last character of a Loop or If statement that doesn't support OTB. E.g. "If x = {" or "Loop, Parse, Var, {"
...
» Dropped Files: Contents will be indented and copied to a new file with a user defined extension (overwrites old file if nothing is specified).

» Hotkey (F2): Highlighted syntax in an editor will be indented, if nothing is highlighted all text will be indented. (thanks ChrisM)
    ⮞ Gui remembers last position and settings between sessions (thanks Rajat)
    ⮞ The case of subroutine calls and function calls is adjusted to the case of their respective definitions
...
Modify AHKPath to the script directory, and the syntax file directory to %AHKPath%\Syntax. bug #1
 ➤ 采用截止今日最新的SCITE4AHK的ahk.api文件，与1.0.48.5的语法文件进行合并。
Adopt the ahk.api file of the latest SCITE4AHK as of today, and merge it with the grammar file of 1.0.48.5.
 ➤ 保留旧的哪怕被移除了的命令，例如REPEAT。这样做的目的是为了支持代码格式化的正常工作。
Keep old commands like REPEAT even if they are removed. The purpose of this is to support the normal working of code formatting.
 ➤ 修改默认不对KEYWORDS进行大小写纠错。bug #2
Modify the default case error correction for KEYWORDS. bug #2
 ➤ 因为目前程序不够智能，当DLLCALL中函数名符合KEYWORDS时，也会进行纠错，而DLLCALL对大小写是敏感的，因此非常容易造成问题。
Because the current program is not smart enough, when the function name in DLLCALL matches KEYWORDS, it will also perform error correction, and DLLCALL is sensitive to case, so it is very easy to cause problems.

 ▔▔▔ 2013.11.22
 ➤ 修改代码，使其运行时直接格式化选中的或全部代码。
Modify the code to directly format selected or all codes at runtime.

 ▔▔▔ 2014.03.06
 ➤ 修改代码，使其支持对for语句的格式化。feature #1
Modify the code to support the formatting of the For statement. feature #1

 ▔▔▔ 2014.03.07
 ➤ 修改代码，使其支持中文函数的格式化。 bug #3
Modify the code to support the formatting of Chinese functions. bug #3
 ➤ 修改默认格式化风格。
Modify the default formatting style.
 ➤ 修改代码，使其支持对While,Until,Try,Catch,Finally语句的格式化。feature #1
Modify the code to support formatting of While, Until, Try, Catch, and Finally statements. feature #1

 ▔▔▔ 2014.03.15
 ➤ 修改代码，Until后面一句不应该缩进
Modify the code, the sentence after Until should not be indented

 ▔▔▔ 2014.03.26
 ➤ 修改代码，改进大小写纠正的问题，以下单词总被纠正为全小写。feature #2
Modify the code to improve the case correction problem, the following words are always corrected to all lowercase. feature #2
      ⮞ if
      ⮞ else
      ⮞ goto
...
Fix nasty bug that would drop lines like "++num". bug #4
 ➤ 对操作剪贴板可能出现的bug做了处理。bug #5
The bugs that may occur in the operation of the clipboard have been dealt with. bug #5
 ➤ 函数后面的花括号（花括号本身）不缩进。feature #3
Curly braces after a function (the braces themselves) are not indented. feature #3
 ➤ 修改版本号为v13。
The modified version number is v13.

 ▔▔▔ 2020.09.03
 ➤ 支持对switch,class语句的格式化。feature #1
Supports formatting of switch and class statements. feature #1
 ➤ 大小写校正存在级别，从低到高依次是，Keywords、Keys、Variables、CommandNames、ListOfDirectives
The existence level of case correction, from low to high, is Keywords, Keys, Variables, CommandNames, ListOfDirectives
 ➤ 如果低级别和高级别单词存在重复，那么总会被高级别覆盖。
If there is a duplication of low-level and high-level words, they will always be overwritten by the high-level.
 ➤ 例如Keys中存在单词“click”，CommandNames中也存在单词“Click”，那么大小写最终就会被纠正成后者。feature #4
For example, the word "click" exists in Keys, and the word "Click" exists in CommandNames, then the case will eventually be corrected to the latter. feature #4
 ➤ 修复 Variables 列表的纠正。bug #6
Fix correction of Variables list. bug #6
 ➤ 更新了语法文件。
